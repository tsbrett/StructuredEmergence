...
class MySimulator:
    ...
    def secondary_infection(self, state,time):
	    #r = np.array([beta[i,:]*state[i] for i in range(state)])
	    return((self.beta.T*state*time).T)
    
    def recovery_vector(self,state):
	    return(self.gamm*state)

    ## Vector determining rate at which class j aquires infection:
    def infection_vector(self,state,time):
        sec_inf = np.sum(self.secondary_infection(state,time), axis =0)
        inf_rate = sec_inf + self.eta
        return(inf_rate)
        
    def rates(self, state, time):
        return(np.concatenate([self.infection_vector(state,time), self.recovery_vector(state)]))
    ...
    def simulate(self):
        ...
        if(np.size(self.eta) != N_classes or np.size(self.gamm) != N_classes or np.size(self.beta,0) != N_classes or np.size(self.beta,1) != N_classes):
       	...
       	N_reactions = len(self.rates(self.initial_condition,0))
        ...
        D = np.zeros(N_reactions)
       	t = 0.
       	te = 0.
       	t_max = self.time_step*self.number_time_steps
       	q = 0
